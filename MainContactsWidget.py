...
class AddFriendMessageBox(MessageBoxBase):
    ...
    def __init__(self, parent=None):
        ...
        self.usernameLineEdit.setPlaceholderText('输入好友的用户名或ID(非昵称)')
    ...
    def add_friend_error(self,data):
        ...
        json_data = data
        QMessageBox.warning(self,"请求失败",json_data["message"])
        self.yesButton.setDisabled(False)
    ...
    def add_friend_request_success(self,data):
        ...
        QMessageBox.information(self,"请求成功","已成功发送请求！")
        DataManager.dataManager.websocket_thread.add_friend_error.disconnect(self.add_friend_error)
        DataManager.dataManager.websocket_thread.add_friend_request_success.disconnect(self.add_friend_request_success)
        self.close()
...
class AddGroupMessageBox(MessageBoxBase):
    ...
    def __init__(self, parent=None):
        super().__init__(parent=parent)
        self.titleLabel = SubtitleLabel('加入群组', self)
        self.groupIDLineEdit = LineEdit(self)

        self.groupIDLineEdit.setPlaceholderText('输入群组ID')
    ...
    def accept(self) -> None:
        ...
        QThreadPool.globalInstance().start(f)
        self.yesButton.setDisabled(True)
        self.yesButton.setText("发送请求中...")
class CreateGroupMessageBox(MessageBoxBase):
    """ Custom message box """
    CreateGroupSuccess = pyqtSignal(dict)
    def __init__(self, parent=None):
        super().__init__(parent=parent)
        self.titleLabel = SubtitleLabel('创建群组', self)
        self.groupNameLineEdit = LineEdit(self)

        self.groupNameLineEdit.setPlaceholderText('输入创建的群组名')
...
class MainContactsWidget(Ui_MainContactsPanel,QWidget):
    ...
    def _initUI(self):
        ...
        self.addSubInterface(self.page_nowChatting,"page_nowChatting","聊天")
        self.addSubInterface(self.page_friends,"page_contacts","联系人")
        self.addSubInterface(self.page_groups,"page_groups","群聊")
        ...
        self.lv_nowChatting.setVerticalScrollBarPolicy(Qt.ScrollBarAlwaysOff)  # 禁用垂直滚动条
        self.lv_nowChatting.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)  # 禁用水平滚动条
        self.lv_nowChatting.setResizeMode(QListWidget.Adjust)
        self.lv_friends.setVerticalScrollBarPolicy(Qt.ScrollBarAlwaysOff)  # 禁用垂直滚动条
        self.lv_friends.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)  # 禁用水平滚动条
        self.lv_friends.setResizeMode(QListWidget.Adjust)
        self.lv_groups.setVerticalScrollBarPolicy(Qt.ScrollBarAlwaysOff)  # 禁用垂直滚动条
        self.lv_groups.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)  # 禁用水平滚动条
        ...
        create_group_action = QAction(FluentIcon.LABEL.icon(), '创建群组')
    ...
    def AddFriend(self,json_data):
        ...
        if message:
            ...
            chatItem.setMessage("已添加好友，可以聊天了")
